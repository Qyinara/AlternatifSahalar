document.addEventListener("DOMContentLoaded", () => {
  ...
  const weightsSummary = [0.23, 0.14, 0.07, 0.22, 0.07, 0.11, 0.14,  0.03];
  ...
  function updateTable(param, table) {
    ...
    const averageEigenvector = eigenvectors.map(
      (row) => row.reduce((sum, value) => sum + value, 0) / n
    );

    rows.forEach((row, rowIndex) => {
      const eigenvalue = row.reduce(
        (sum, cellValue, colIndex) =>
          sum + cellValue * averageEigenvector[colIndex],
        0
      );
      ...
      const normalizedCell =
        tbody.rows[rowIndex].querySelector(".normalized-matrix");
      normalizedCell.textContent = eigenvectors[rowIndex]
        .map((v) => v.toFixed(2))
        .join(", ");

      const eigenvectorCell =
        tbody.rows[rowIndex].querySelector(".eigenvector");
    ...
    totalRow.querySelector(".lambda-max").textContent =
      lambdaMaxAverage.toFixed(2);
  ...
  function renderSummaryTable() {
    ...
    alternatives.forEach((alt, index) => {
      ...
      tableParams.forEach((param) => {
        const td = document.createElement("td");
        const paramSum = tableData[param][index]
          .reduce((a, b) => a + b, 0)
          .toFixed(2);
      ...
      eigenvectorCell.textContent = weightsSummary[index];

      row.appendChild(eigenvectorCell);

      const weightCell = document.createElement("td");
      weightCell.classList.add(`total-weight-${rowIndex}`);
  ...
  function updateResults() {
    ...
    tableParams.forEach((param, tableIndex) => {
      ...
      for (let i = 0; i < alternatives.length; i++) {
        const cellValue = document.getElementsByClassName(
          `${tableIndex}-eigenvector${i}`
        );
    ...
    tableParams.forEach((param) => {
      ...
      if (paramValues) {
        paramValues.forEach((value, index) => {
          const resultCell = document.getElementsByClassName(
            `result-${param}${index}`
          );
    ...
    const weights = [0.23, 0.14, 0.14, 0.11, 0.12, 0.14, 0.11, 0.1]; // Her parametreye ait ağırlıklar
    ...
    totalWeights.forEach((weight, index) => {
      const weightCell = document.getElementsByClassName(
        `result-Toplam Ağırlık Değerleri${index}`
      );
    ...
    totalWeights.forEach((weight, index) => {
      const potentialCell = document.getElementsByClassName(
        `result-Potansiyel Ağaçlandırılacak Saha${index}`
      );
    ...
    for (let x = 0; x < 8; x++) {
        debugger
      let totalWeight = 0;
      tableParams.map((tableParam, subIndex) => {
        let currentAreaValue = document.getElementsByClassName(
          `result-${tableParam}${x}`
        );
        if (currentAreaValue.length > 0) {
          let floatValue =
            parseFloat(currentAreaValue[0].textContent) *
            weightsSummary[subIndex];
          if (floatValue) {
            totalWeight += floatValue;
          }
        }
      });

      currentTotal = document.getElementsByClassName(`total-weight-${x}`);
      if (currentTotal.length>0) {
        currentTotal[0].textContent = totalWeight;
      }
    }
  }
